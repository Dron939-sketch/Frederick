...
def test_no_strikethrough_price_anywhere():
    ...
    «290 ₽». Скидки в продукте нет: и месяц, и проба стоят одинаково
    в пересчёте на день, а зачёркнутая цена без скидки — недостоверная
    реклама (ст. 5 ФЗ «О рекламе»). Подарок решает ту же задачу честно,
    поэтому фиксируем: цены 290 в коде нет.
    """
