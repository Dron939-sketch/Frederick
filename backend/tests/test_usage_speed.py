...
Показанный запас первого разговора расходуется быстрее настоящих часов:
каждая реальная секунда списывается с коэффициентом USAGE_SPEED. Место
...
Проверка идёт от констант, а не от вписанных руками «десяти и семи»:
23.09.2026 запас удлинили с 10 до 20 минут, и тест, знавший семь минут
наизусть, упал на изменении, которое проверять не должен был.
"""
...
def test_shown_minutes_are_spent_faster_by_the_multiplier():
    m = _meter()
    ...
    while spent < window:
        ...
        real += 30
    expected = window / m.USAGE_SPEED
    assert abs(real - expected) <= 45, (
        f"показанный запас {m.FIRST_CONVERSATION_MINUTES} мин при коэффициенте "
        f"{m.USAGE_SPEED} должен кончаться примерно за {expected/60:.1f} реальных минут, "
        f"вышло {real/60:.1f}")
    assert real < window, "без ускорения списания тест ничего не проверяет"
