...
_B2C_TAIL_SYSTEM = (
    ...
    "регистрацию на входе (email + 4-значный пин-код). Это короткая, "
    "но регистрация. Расхождение с лендингом убивает доверие.\n"
...
_B2C_VOICE_SYSTEM = (
    ...
    "  5. ЧЕСТНЫЙ ВЫХОД (8–12 сек): «Десять минут в день — бесплатно. "
    "На входе короткая регистрация: email и четырёхзначный пин, чтобы "
    "прогресс не терялся. Если зайдёт — будешь возвращаться. Если "
    "нет — ничего не теряешь».\n\n"
    "ЖЁСТКО:\n"
    ...
    "регистрация (email + четырёхзначный пин). Это правда, и об этом "
    "надо сказать прямо: «На входе короткая регистрация — email и "
    "четырёхзначный пин, чтобы прогресс не терялся».\n"
...
def _fallback_voice(category_meta: Dict[str, Any], first_name: str,
    ...
    if b2c_mode:
        ...
        return (
            ...
            f"Десять минут в день — бесплатно. На входе короткая регистрация: "
            f"email и четырёхзначный пин, чтобы прогресс не терялся. Если "
            f"зайдёт — будешь возвращаться. Если нет — ничего не теряешь."
        )
