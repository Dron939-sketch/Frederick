...
_TEST_ACCEPT_RE = re.compile(
    # Узкий список — только однозначно согласительные слова. Раньше тут были
    # «тест», «поехали», «открой» — они срабатывали в посторонних контекстах
    # («опять ты свой тест предлагаешь», «открой окно», «поехали в кино»).
    r"\b(да|давай|давайте|ок|окей|конечно|ага|хочу|можно|попробую|попробуем|"
    r"начнём|начнем|готов|готова|запускай)\b",
    re.IGNORECASE,
)
_TEST_DECLINE_RE = re.compile(
    r"(\bне\s+(?:хочу|надо|сейчас|буду|готов|готова)\b|\bнет\b|\bпотом\b|\bотстань\b|\bпозже\b|"
    r"\bотмен[аяи]\w*|\bхватит\b|\bдостал[аои]?\b|\bсраный\b)",
    re.IGNORECASE,
)


def _is_test_acceptance(text: str) -> bool:
    """True, если сообщение пользователя — согласие на оффер теста.

    Используется только когда `basic_test_offered=True`: после оффера на 4-м
    сообщении BasicMode мы должны поймать «да/давай/попробую/...» и сообщить
    фронту `action=open_test`, чтобы открыть тест-интерфейс.

    Длинные сообщения (жалобы, обсуждения, дискуссии) НЕ считаются согласием,
    даже если содержат «да» — иначе фраза вроде «Да, у тебя время бежит в 2
    раза быстрее» открывала тест на ровном месте.
    """
    if not text:
        return False
    cleaned = text.strip()
    # Согласие — короткое сообщение, не текст-простыня.
    if len(cleaned) > 40:
        return False
    if _TEST_DECLINE_RE.search(cleaned):
        return False
    return bool(_TEST_ACCEPT_RE.search(cleaned))
...
async def _enforce_premium_mode(user_id, requested_mode: str) -> str:
    ...
    с урезанной версией и по ней решал, стоит ли платить за ту,
...
_METER_AI_REGEX = _re_meter.compile(
    ...
    # mirrors/complete здесь НЕ стоит и стоять не должен: это чистая запись
    # в fredi_mirrors, ни одного токена. Ручка вызывается в самом конце
    # теста — то есть ровно тогда, когда пятнадцать минут уже потрачены и
    # бесплатный лимит у приглашённого кончился. Пока она была под meter,
    # друг проходил тест целиком, а зеркало молча не активировалось: 402,
    # пригласивший не получал ничего. LLM в зеркалах живёт в
    # mirrors/<code>/intimate — это GET, и meter (POST-only) его и так
    # не видит; если понадобится считать и его, метод придётся расширять
    # отдельно, а не возвращать сюда complete.
    r"|morning/send-now"
