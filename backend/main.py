...
async def deep_analysis(request: Request, data: ChatRequest):
    ...
    try:
        ...
        # ИСПРАВЛЕННЫЙ ПРОМПТ С ТРЕБОВАНИЯМИ ФОРМАТИРОВАНИЯ
...
- Для жирного текста используй **две звездочки** (пример: **важный текст**)
- НЕ используй одинарные звездочки * для жирного текста
- Для заголовков используй ## (две решетки) и ставь ПРОБЕЛ после них
- После каждого заголовка ОБЯЗАТЕЛЬНО делай перенос строки
- Для списков используй * (звездочку) в начале строки
- Каждый пункт списка с новой строки
- Между разными разделами делай пустую строку
...
Структура ответа (строго соблюдай заголовки и эмодзи):

## 🔍 ГЛУБИННЫЙ ПОРТРЕТ
Текст анализа... Напиши 3-4 предложения.
...
## 📊 ПРОГНОЗ
**Без изменений:** Что будет, если ничего не менять...
**При работе над собой:** Что станет возможным...
...
ВАЖНО:
- Не склеивай текст с заголовками
- Всегда ставь перенос строки после заголовка
- Пиши на русском языке, грамотно, с пробелами между словами
- Обращайся к пользователю на "ты"
- Будь честным, но бережным"""

        user_prompt = f"""
Данные пользователя для глубокого анализа:
...
Проведи глубокий психологический анализ по указанной структуре. Используй **жирный текст** для важных акцентов. НЕ используй одинарные звездочки.
"""
