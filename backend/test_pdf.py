...
def _clean_for_pdf(text: str) -> str:
    """Удаляет эмодзи и HTML-теги, схлопывает образовавшиеся пробелы.

    Теги — страховка. В PDF идёт сырой текст из базы, а разметку в <b>
    навешивает format_psychologist_text уже на выходе к экрану. Но путей
    к отчёту несколько, и стоит кому-то передать сюда готовый к показу
    текст — на бумаге напечатается «<b>КЛЮЧЕВОЙ ЭЛЕМЕНТ</b>» словами.
    Заодно чинится распознавание заголовков: «<b>ЗАГОЛОВОК</b>» не
    проходит проверку «все буквы заглавные» из-за латинской b в теге.
    """
    if not text:
        return ""
    s = re.sub(r"<[^>]+>", "", text)
