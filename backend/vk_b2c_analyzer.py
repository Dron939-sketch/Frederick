...
_PAIN_SYSTEM = (
    ...
    "  • 🚨 evidence_quotes ДОЛЖНЫ БЫТЬ БОЛЕВЫМИ. НЕ включай:\n"
    "      - позитивные/декларативные («не доказываю», «я сильная»)\n"
    "      - маркетинговые/продуктовые («делаем приложение для\n"
    "        миллионов», «приходите на тренинг»)\n"
    "      - праздничные/нейтральные («с днём рождения», «прекрасный\n"
    "        день»)\n"
    "    Лучше пустой evidence_quotes, чем заполненный позитивом —\n"
    "    pitch потом цитирует это как «боль», что разрушает доверие.\n"
...
_HOOK_SYSTEM = (
    ...
    "  • 🚨 НЕ цитируй позитивные / самоутверждающие / декларативные\n"
    "       фразы (например «не доказываю что хорошая»). Это РЕСУРС,\n"
    "       а не боль. Цитируй ТОЛЬКО маркеры боли (усталость, сомнение,\n"
    "       разрыв, противоречие, признание слабости).\n"
    "  • 🚨 НЕ цитируй маркетинговые / продуктовые фразы (например\n"
    "       «делаем приложение которое помогает миллионам»). Это\n"
    "       профессиональная декларация, а не личный материал.\n"
    "  • 🚨 Если в постах НЕТ ни одной болевой цитаты — лучше не\n"
    "       цитируй вообще, чем цитируй позитив/маркетинг.\n\n"
...
async def analyze_profile(url_or_name: str) -> Dict[str, Any]:
    ...
    #
    # 🚨 ЗАЩИТА ОТ LLM-ДРЕЙФА: все 10 шаблонов journey написаны под
    # женщину 30+ ЦА (feminine окончания, мать-дочь, бьюти-контекст).
    # Если LLM ослушался гейта в hint'е и вернул journey для мужчины
    # или не-ЦА — мы НЕ резолвим. Лучше отсутствие journey, чем
    # «ты в детстве не выбирала» мужчине.
    ...
    try:
        if isinstance(pain, dict):
            ...
            # ГЕЙТЫ
            _is_target = bool(pain.get("is_target_audience"))
            _cp = (pain.get("compensatory_pattern") or "none").strip().lower()
            _gender_ok = (gender == "f")
            _gates_ok = _is_target and _cp != "none" and _gender_ok
            if jcode and jcode != "none" and jweight >= 0.4 and _gates_ok:
