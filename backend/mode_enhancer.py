...
# вообще НЕ зовём LLM.
# 10.09.2026. Правило «ключевое слово где-то в тексте И второе лицо
# где-то в тексте» ловило обычную речь. В выгрузке одного разговора на
# 126 сообщений канонический ответ подменил живой шесть раз, и один из
# них — на просьбе «Соседка заявила, что я взяла её ведёрко. Помогите
# пожалуйста мне». Человек просил помощи и получил «Меня создали Андрей
# Мейстер и Андрей Соколов», потому что в сообщении были слова «Сделала»
# и «Вашему».
#
# Причина в двух вещах. Первая: «сдела(л|ли)» ловит «сделал», «сделала»,
# «сделали» — одно из самых частых слов языка, и к вопросу об авторстве
# оно почти никогда не относится («я сделала упражнение», «что он сделал»).
# Убрано целиком: «кто тебя сделал» и так закрывается через «созда».
# Вторая: два условия проверялись по всему тексту, поэтому в длинном
# рассказе они встречались случайно. Теперь второе лицо должно стоять
# рядом с ключевым словом — в окне ±40 символов.
#
# И порог длины: вопрос «кто тебя создал» короткий. Длинный рассказ,
# в котором эти слова встретились, — это рассказ, а не вопрос.
_ID_KEYWORD_RE = re.compile(
    r"(созда(?:л|тель|теля|ли|вш)|разработ(?:ал|ан|чик|чика|ала|ки|чиком)|"
    r"\bавтор\w*|\bмодел[ьи]\b|нейросет|движок|обучал\w*|"
    r"за\s+тобой\s+стои)",
    re.IGNORECASE,
)
_ID_SECONDPERSON_RE = re.compile(
    r"\b(теб[яе]|тобой|тво[йяеёию]\w*|ты|своего|своё|свой|своя|ваш\w*|вас|вами)\b",
    re.IGNORECASE,
)

# Дальше этой длины перехват не работает: это уже не вопрос об авторстве.
_ID_MAX_LEN = 200
# На столько символов вокруг ключевого слова ищем второе лицо.
_ID_NEAR = 40
...
def _is_identity_question(text: str) -> bool:
    ...
    for m in _ID_KEYWORD_RE.finditer(t):
        lo = max(0, m.start() - _ID_NEAR)
        hi = min(len(t), m.end() + _ID_NEAR)
        if _ID_SECONDPERSON_RE.search(t[lo:hi]):
            return True
    return False
