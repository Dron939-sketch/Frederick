...
class PsychologistMode(BaseMode):
    ...
    async def _crisis_response(self) -> AsyncGenerator[str, None]:
        """Кризисный ответ. Фреди остаётся опорой и держит контакт — но при
        суицидальном риске обязан дать живого человека рядом с собой, а не
        вместо себя. Прежняя версия сознательно не называла телефон доверия;
        02.09.2026 это обернулось реальным диалогом, где человеку с мыслями
        о смерти номер так и не прозвучал. Теперь номер — обязательная часть
        ответа: коротко, без паники, и разговор продолжается."""
        yield (
            "Я тебя услышал. То, что ты сейчас сказал — серьёзно, и я не хочу "
            "оставлять тебя с этим одного. Я здесь, и мы продолжим разговор.\n\n"
            "Но сначала одна важная вещь. Когда есть мысли о том, чтобы не жить, "
            "рядом должен быть живой человек, а не только я. Телефон доверия "
            "8-800-2000-122 — бесплатно, круглосуточно и анонимно; там просто "
            "выслушают, это ни к чему не обязывает. Если станет совсем остро — "
            "звони 112. Сохрани эти номера прямо сейчас, это займёт десять секунд.\n\n"
            "А теперь — я рядом, и мне важно понять, что происходит. Эти мысли "
            "появились именно сегодня или это уже какое-то время? Есть что-то "
            "конкретное, что подталкивает их сейчас? И что или кто рядом с тобой "
            "сегодня удерживает тебя на плаву?"
        )
