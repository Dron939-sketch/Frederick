...
class BasicMode(BaseMode):
    ...
    Работает строго по системному промпту. Без лишнего анализа.
    """
...
Мягко и с юмором подводи к тесту, но не дави."""
        ...
        try:
            async for chunk in self.ai_service.call_deepseek_streaming(
                ...
                if clean_chunk.strip():
                    yield clean_chunk
                    await asyncio.sleep(0.015)   # плавность речи
    ...
    def _build_clean_prompt(self, question: str) -> str:
        address = self._get_address()
        history = "\n".join(self.conversation_history[-12:])  # последние 12 сообщений
