...
QUICK_RULES: Dict[str, List[str]] = {
    "cbt": [
        ...
        # Action-запросы: «что мне делать», «как быть», «помоги решить» —
        # лучше всего работают через CBT (структурный action plan).
        r"\bчто (мне )?делать\b", r"\bкак (мне )?быть\b",
        r"\bпомоги (мне )?(решить|разобраться|справ\w*)",
        r"\bподскажи\b", r"\bкак (мне )?поступ\w*",
        r"\bхочу (что-то )?(сделать|изменить|решить)",
        r"\bне знаю что делать\b", r"\bруки опускаются\b",
        r"\bопускаются руки\b", r"\bапати\w*",
    ],
    "logo": [
        ...
        # Безысходность, развод, «всё рушится» — Франкл лучше Роджерса
        # вытягивает из «всё пропало» к «ради чего стою».
        r"\bбезысходн\w*", r"\bвсё (пропало|рушится|потеряно)\b",
        r"\bтеряю всё\b", r"\bжизнь (рушится|закончена)\b",
        r"\bничего (не )?хочется\b", r"\bне вижу выход\w*",
    ],
    "existential": [
    ...
    "transactional": [
        ...
        r"\bэго-состоян\w*",
        # Манипулятивные/токсичные/абьюзивные отношения — это
        # классические TA-игры (Карпман, "Now I Got You", "Yes But").
        # Сюда же: газлайтинг, нарциссы, психопаты, желание мести,
        # «приполз/приползёт» — характерная фраза после манипулятора.
        r"\bманипулятор\w*", r"\bманипуляц\w*",
        r"\bабьюз\w*", r"\bабьюзив\w*", r"\bабьюзер\w*",
        r"\bтоксичн\w* отношени\w*", r"\bтоксичн\w* (партнёр|муж|жена|парень|девушк)\w*",
        r"\bгазлайт\w*", r"\bгазлайтинг\w*",
        r"\bнарцис\w*", r"\bпсихопат\w*",
        r"\bотомст\w*", r"\bмест\w*",  # хочу отомстить / месть
        r"\bпричинить ему боль\b", r"\bпрополз\w*", r"\bприполз\w*",
        r"\bкак (его|её) (вернуть|заставить|наказать)\b",
        r"\bпусть он почувству\w*", r"\bпусть она почувству\w*",
    ],
}
...
# Сужено: только реальные суицидальные/жизнеугрожающие маркеры.
# «Помогите», «кризис», «не могу больше» — это запросы помощи, а не повод
# сбрасывать разговор в шаблон. На них Фреди должен работать как обычно
# (через CBT/logo/Rogers), но особенно внимательно.
CRISIS_MARKERS: List[str] = [
    # Порядок слов в живой речи свободный: «не хочется жить» и «жить не
    # хочется» — одна и та же фраза. 02.09.2026 второй вариант прошёл мимо
    # фильтра в реальном диалоге («вообще жить не хочется») — поэтому обе
    # перестановки и вставные слова («больше», «так», «вообще») учтены.
    r"\bне хоч(?:у|ется)\s+(?:больше\s+|так\s+|вообще\s+)?жить\b",
    r"\bжить\s+(?:больше\s+|так\s+|вообще\s+)?не\s+хоч(?:у|ется)\b",
    r"\bжить\s+надоело\b", r"\bнадоело\s+жить\b",
    r"\bне\s+вижу\s+смысла\s+жить\b",
    r"\bсамоубийств\w*", r"\bпокончить\s+с\s+(?:собой|жизнью)\b",
    r"\bпокончу\s+с\s+собой\b", r"\bпокончил\w?\s+бы\s+с\s+собой\b",
    r"\bхочу умереть\b", r"\bлучше (бы )?умереть\b",
    r"\bсуицид\w*",
    r"\bзаписку (написал|написала|оставил|оставила)\b",
    r"\bтаблетк\w* выпью\b", r"\bвены вскрою\b",
    r"\bпрыгну с\b",
    r"\bпричинить\s+себе\s+вред\b", r"\bпорезать\s+себя\b",
]
...
- transactional — сценарии в отношениях, роли, «игры», повторяющиеся
  ...
  ОСОБЕННО СЮДА: манипулятивные/абьюзивные/токсичные отношения, газлайтинг,
  нарциссы, желание мести бывшему, «как его вернуть/заставить» —
  это всё ТА-игры, и анализ игр Берна — самый точный инструмент.
...
async def classify(
    ...
    # Шаг 4: fallback. Раньше был Роджерс — но это создавало проблему
    # «бесконечного отражения без действия» на любой неопределённый запрос.
    # Теперь по умолчанию CBT (Beck) — у него встроена структура, action plan
    # и психообразование, что для большинства случаев продуктивнее.
    if "cbt" not in exclude_methods:
        return {
            "method_code": "cbt",
            "confidence": 0.4,
            "reason": "fallback: action-oriented подход для общего запроса",
            "source": "fallback",
        }
    # Если CBT тоже исключён — Роджерс как последний резерв.
    return {
        "method_code": "person_centered",
        "confidence": 0.4,
        "reason": "fallback: CBT исключён, остался person_centered",
        "source": "fallback",
    }
