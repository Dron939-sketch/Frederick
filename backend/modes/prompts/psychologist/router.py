...
CRISIS_MARKERS: List[str] = [
    ...
    # Ниже — формулировки, которых фильтр не ловил до 03.09.2026. Проверены
    # на живых обращениях: подросток скорее напишет «хочу сдохнуть», чем
    # «хочу умереть», а взрослый — «уйти из жизни» или «не хочу просыпаться».
    # Цена ложного срабатывания — один раз за сессию показанный телефон,
    # цена пропуска — несопоставима, поэтому список намеренно широкий.
    r"\bхочу\s+сдохнуть\b", r"\bсдохнуть\s+(?:бы\s+)?хочу\b",
    r"\bлучше\s+(?:бы\s+)?сдохнуть\b",
    r"\bхочу\s+ум(?:е|и)реть\b",
    r"\bуйти\s+из\s+жизни\b", r"\bуход\s+из\s+жизни\b",
    r"\bсвести\s+счёты\s+с\s+жизнью\b", r"\bсвести\s+счеты\s+с\s+жизнью\b",
    r"\bне\s+хоч(?:у|ется)\s+(?:больше\s+)?просыпаться\b",
    r"\bне\s+проснуться\s+(?:бы\s+)?(?:завтра|утром|больше)\b",
    r"\bхочу\s+(?:просто\s+|навсегда\s+)?исчезнуть\b",
    r"\bменя\s+не\s+было\s+бы\b", r"\bлучше\s+бы\s+меня\s+не\s+было\b",
    r"\bне(?:за|\s+за)чем\s+жить\b", r"\bсмысла\s+жить\s+нет\b",
    r"\bустал\w?\s+жить\b",
    r"\bне\s+хоч(?:у|ется)\s+(?:больше\s+)?существовать\b",
    r"\bреж(?:у|ю)\s+себя\b", r"\bрезать\s+себя\b",
    r"\bсамоповрежд\w*", r"\bбью\s+себя\b",
]
...
# ============================================================
# КРИЗИСНЫЙ ПЕРЕХВАТ ДЛЯ ВСЕХ РЕЖИМОВ
# ============================================================

# Фильтр жил только в BasicMode и PsychologistMode. Но человек в кризисе
# пишет туда, где оказался: 03.09.2026 в разборе обращений нашлась
# шестнадцатилетняя, писавшая Коучу. Режим не должен решать, прозвучит
# телефон доверия или нет, поэтому текст и правило вынесены сюда — их
# используют Коуч и Тренер, у которых своего кризисного ответа не было.
CRISIS_NOTICE = (
    "Остановлюсь. То, что вы сейчас написали, важнее задачи, ради которой мы "
    "здесь. Я никуда не денусь и мы продолжим — но сначала одно.\n\n"
    "Когда появляются мысли не жить или причинить себе вред, рядом должен "
    "быть живой человек. Телефон доверия 8-800-2000-122 — бесплатно, "
    "круглосуточно, анонимно. Если совсем остро — 112.\n\n"
    "Скажите, эти мысли появились сегодня или уже какое-то время с вами?"
)


def crisis_reply(mode, question: str):
    """Вернуть кризисный текст, если он нужен, иначе None.

    Номер даётся один раз за сессию: повторять его на каждое сообщение —
    значит превратить поддержку в шаблонную отписку. Флаг хранится и на
    инстансе, и в user_data, потому что инстанс режима создаётся заново
    на каждый HTTP-запрос."""
    try:
        if not has_crisis_marker(question):
            return None
        shown = (
            getattr(mode, "_crisis_said", False)
            or bool(getattr(mode, "user_data", {}).get("crisis_notice_shown"))
        )
        if shown:
            return None
        mode._crisis_said = True
        try:
            mode.user_data["crisis_notice_shown"] = True
        except Exception:
            pass
