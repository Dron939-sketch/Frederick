...
# месяцем сразу (он тогда стоил 990 ₽) нет ступеньки. Ступенькой стала
# проба: полный Premium, карта сохраняется, дальше обычный месяц
# автопродлением (отключается в один клик). Проба — один раз на аккаунт:
# тому, у кого уже была любая подписка, не продаётся.
#
# 24.09.2026 месяц снижен с 990 до 690 ₽ решением владельца. Повод:
# с 16.09 по 24.09 стену оплаты увидели 155 человек, кликнул один,
# оплат ноль; снижение пробы с 99 до 69 ₽ 22.09 ничего не изменило,
# так что проверяется вторая цена. Автопродление считается из той же
# константы, поэтому следующее списание у тех, кто уже подписан,
# пройдёт по новой цене — это осознанно.
#
...
def plan_price(plan: Optional[str] = "monthly") -> int:
    """Цена тарифа целым числом рублей — для текстов, которые читает человек.

    22.09.2026 цену меняли руками, и шесть мест на экране подписки остались
    со старой: человек видел одно число, касса списывала другое. Любой
    текст про цену берёт её отсюда, а не вписывает цифру.
    """
    return int(round(float(plan_info(plan)["amount"])))
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                resp = await client.post(
                    ...
                    headers={
                        ...
                        # списание месячной цены. Повтор на следующий день —
            ...
            rows = await conn.fetch(f"""
                ...
                WHERE s.auto_renew = TRUE
                  ...
                  AND (
                    ...
                    -- месячное списание на шестой день из семи нельзя
