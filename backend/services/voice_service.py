...
def restore_punctuation(text: str) -> str:
    ...
    # 10. Убираем множественные знаки препинания в конце (ИСПРАВЛЕНО!)
    # Если в конце несколько одинаковых знаков, оставляем один
    text = re.sub(r'([.!?])\1+$', r'\1', text)
    # Если в конце несколько разных знаков, оставляем последний
    if len(text) > 1 and re.search(r'[.!?]{2,}$', text):
        last_char = text[-1]
        text = re.sub(r'[.!?]{2,}$', last_char, text)
