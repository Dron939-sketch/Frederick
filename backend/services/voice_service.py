...
def normalize_tts_text(text: str) -> str:
    ...
    Убирает:
    - разрыв слов пробелами (В е ч е р → Вечер)
    - лишние пробелы
    - эмодзи
    - спецсимволы
    """
    ...
    # 2. Убираем все спецсимволы, кроме базовых знаков препинания
    text = re.sub(r'[#*_`~<>|@$%^&+={}[\]\\;:]', ' ', text)

    # 3. ГЛАВНОЕ: склеиваем разорванные буквы (В е ч е р → Вечер)
    #    но НЕ трогаем нормальные слова
    text = re.sub(r'([а-яёА-ЯЁa-zA-Z])\s+([а-яёА-ЯЁa-zA-Z])', r'\1\2', text)

    # 4. Убираем лишние пробелы
    text = re.sub(r'\s+', ' ', text)

    # 5. Пробелы после знаков препинания
