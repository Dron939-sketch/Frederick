...
ВЕРСИЯ 3.3 — С ПОДДЕРЖКОЙ ВОКАЛЬНЫХ МАРКЕРОВ
...
def process_vocal_markers(text: str) -> str:
    ...
    Поддерживает:
    - [вздох], [вздыхает] → пауза с многоточием
    - [смех], [смеётся] → ремарка о смехе
    - [пауза], [молчание] → пауза
    - [задумчиво] → пауза-размышление
    - [радостно] → восклицание
    - [удивлённо] → вопросительный знак с восклицанием
    - [грустно] → многоточие
    - [иронично] → ремарка об иронии
    """
    if not text:
        return text
...
def restore_punctuation(text: str) -> str:
    ...
    Добавляет точки, запятые, вопросительные знаки.
    """
    if not text:
        return text
    ...
    # 14. Убираем множественные знаки препинания в конце
    ...
    # 16. Проверка: если текст стал короче 3 символов, возвращаем оригинал
    if len(text) < 3:
        return original
...
def normalize_tts_text(text: str) -> str:
    ...
    # 2. Убираем спецсимволы, НО СОХРАНЯЕМ знаки препинания
...
class VoiceService:
    ...
    def __init__(self):
        ...
        logger.info("=" * 70)
        logger.info("🎤 VoiceService v3.3 успешно инициализирован")
