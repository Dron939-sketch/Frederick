...
ВЕРСИЯ 3.4 — С ОЧИСТКОЙ ОТ РЕМАРОК
...
- Убирает ремарки без скобок: "делает паузу", "внимательно смотрит"
...
def process_vocal_markers(text: str) -> str:
    """
    Преобразует текстовые вокальные маркеры в знаки препинания и ремарки для TTS.
    """
    if not text:
        return text
...
def restore_punctuation(text: str) -> str:
    ...
    # 8. Добавляем запятые после обращений
    address_words = [
        'друг мой', 'дорогой', 'приятель', 'друг', 'товарищ', 
        'слушай', 'дружок', 'братец', 'уважаемый', 'собеседник',
        'мой друг', 'дорогой друг'
    ]
    for word in address_words:
        text = re.sub(rf'\b{word}\s+', rf'{word}, ', text, flags=re.IGNORECASE)
    
    # 9. Добавляем запятые после вводных слов
    intro_words = [
        'спасибо', 'пожалуйста', 'извините', 'прости', 'знаешь', 
        'понимаешь', 'видишь', 'слышишь', 'представь', 'вообще',
        'кстати', 'наверное', 'конечно', 'действительно', 'честно',
        'например', 'во-первых', 'во-вторых', 'итак', 'однако',
        'ну', 'вот', 'в общем', 'короче'
    ]
    for word in intro_words:
        text = re.sub(rf'\b{word}\s+', rf'{word}, ', text, flags=re.IGNORECASE)
    
    # 10. Добавляем запятые перед союзами "а", "но"
    text = re.sub(r'\s+(а|но)\s+', r', \1 ', text, flags=re.IGNORECASE)
    
    # 11. Добавляем запятые после вопросительных слов в начале
    question_words = ['как', 'что', 'где', 'когда', 'почему', 'зачем', 'кто', 'чей', 'сколько']
    for word in question_words:
        text = re.sub(rf'^{word}\s+', rf'{word}, ', text, flags=re.IGNORECASE)
    ...
    # 13. Убираем двойные пробелы
    text = re.sub(r'\s{2,}', ' ', text)
    
    # 14. Убираем множественные знаки в конце
    if len(text) > 1 and text[-1] in '.!?' and text[-2] in '.!?':
        text = text[:-1]
    
    # 15. Убираем запятую перед точкой
    text = re.sub(r',\s*\.', '.', text)
...
def normalize_tts_text(text: str) -> str:
    ...
    - Убирает ремарки типа "делает паузу, внимательно смотрит"
    - Нормализует числа и даты
    ...
    # ========== УБИРАЕМ РЕМАРКИ (без скобок) ==========
    vocal_phrases = [
        r'делает паузу', r'внимательно смотрит', r'задумывается',
        r'медленно говорит', r'быстро говорит', r'шепотом',
        r'с улыбкой', r'с грустью', r'с иронией', r'смеётся',
        r'вздыхает', r'кашляет', r'молчит', r'пауза'
    ]
    
    for phrase in vocal_phrases:
        text = re.sub(rf'\s*{phrase}\s*[,.]?\s*', ' ', text, flags=re.IGNORECASE)
    
    # Убираем всё, что в квадратных скобках (запасной вариант)
    text = re.sub(r'\[[^\]]*\]', '', text)
    # ===================================================
    ...
    # 4. Обрабатываем вокальные маркеры
    text = process_vocal_markers(text)
    
    # 5. Восстанавливаем знаки препинания
...
class VoiceService:
    ...
    def __init__(self):
        ...
        logger.info("=" * 70)
        logger.info("🎤 VoiceService v3.4 успешно инициализирован")
