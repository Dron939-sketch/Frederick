...
def _trial_note(plan: str) -> str:
    ...
    # с 15.09.2026 это три дня, с 22.09.2026 — за 69 ₽ (см. PLANS в payment.py).
    if plan != "trial_week":
        return ""
    # Цены берутся из PLANS, а не вписываются: 22.09.2026 правка руками
    # разошлась с кассой. Импорт локальный — payment.py сам зовёт этот модуль.
    from payment import plan_price
    return (f"Это пробные три дня за {plan_price('trial_week')} ₽. Когда они закончатся, подписка "
            f"продолжится за {plan_price('monthly')} ₽ в месяц — отключить автопродление можно "
