...
def _trial_text(s: dict) -> str:
    ...
    проверяемые. Цена месяца и безлимит по минутам — из PLANS и subscription_meter
    ...
    return (
        ...
        f"Если хочется без счётчика и с голосом — пробные три дня "
        f"за {plan_price('trial_week')} ₽ (потом {plan_price('monthly')} ₽ в месяц, "
        "отключается в один клик), говорите столько, "
        "сколько нужно. Если пока не время — ничего не потеряется: история "
        "и профиль остаются."
    )
