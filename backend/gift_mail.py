...
отправить письмом два сборника тренировочных заданий «Три пути» —
редакция с легендой курсанта (парень) и с легендой курсантки (девушка),
...
def _paragraphs(name: str) -> List[Any]:
    return [
        _greeting(name),
        ("Вы с подпиской на Фреди, и у меня для вас подарок — не для вас, а для "
         "вашего сына или дочери, если им от шестнадцати до девятнадцати. "
         "К письму приложены два сборника тренировочных заданий «Три пути»: "
         "один с легендой курсанта, для парня, другой с легендой курсантки, "
         "для девушки. Это одно и то же по устройству, разная только жизнь "
...
def html_body(name: str = "") -> str:
    ...
    return ("<!doctype html><html lang=\"ru\"><body style=\"margin:0;padding:24px 16px;"
            ...
            + "".join(ps) +
            "<p style=\"margin:18px 0 0;color:#888;font-size:12px\">Во вложении два PDF: сборник с "
            "легендой курсанта и сборник с легендой курсантки, по 25 страниц А4.</p>"
            "</div></body></html>")
