...
class QuestionContextAnalyzer:
    """
    Анализирует вопросы пользователя в контексте его конфайнтмент-модели
    ...
    def __init__(self, model: ConfinementModel9, user_name: str = "друг"):
        ...
        Args:
            model: конфайнтмент-модель пользователя
        ...
        # Определяем ключевое ограничение (вручную, без импорта confinement_reporter)
    ...
    def _detect_key_confinement(self) -> Optional[Dict[str, Any]]:
        """
        Определяет ключевое ограничение из петель
        """
        if not self.loops:
            return None
        ...
        if strongest_loop:
            ...
            return {
                'description': strongest_loop.get('description', 'Ключевое ограничение'),
                'element': first_elem,
                'impact': max_impact,
                'type': strongest_loop.get('type', 'major_loop')
            }
