"""
subscription_meter.py — платный Фреди: подписка на месяц (цена в PLANS) и возобновляемый
бесплатный уровень.
...
# 12.09.2026: со второго дня аноним без минут. Первый разговор остаётся
# полным (FIRST_CONVERSATION_MINUTES), а возвращаться на второй день — с
# почтой: анонимов второго дня единицы, терять нечего, а почта даёт канал
# для писем возврата и «+5 минут сегодня» в день регистрации.
FREE_DAILY_MINUTES_ANON = 0
...
#
# 23.09.2026: первый разговор удлинён вдвое, 10 → 20 (решение владельца).
# Повод — воронка: за 17–23.09 реклама дала 13 073 ₽ расхода, 219 первых
# сообщений и ноль оплат, а стену оплаты за ту же неделю увидел 131
# человек при одном клике по подписке. Десять минут запаса — это около
# семи настоящих (USAGE_SPEED ниже), и на седьмой минуте разговор
# обрывается раньше, чем человек успевает получить то, за что платят:
# ни привычки, ни боли, ни повода отдать 69 ₽. Своя же выгрузка за
# месяц (см. комментарий выше) говорит ровно об этом: все длинные
# разговоры базы, 20–60 минут, начались с того, что человека не
# прервали.
#
# Дневные 5 минут возврата не тронуты: щедрым делается знакомство, а не
...
def daily_limit_minutes(registered: bool, first_day: bool = False,
    ...
    # а предложение занимало место разговора о пробе за 69 ₽.
...
# Множитель списания (22.09.2026, решение владельца). Человеку обещаны и
# показываются десять минут первого разговора, а расходуются они быстрее:
# каждая реальная секунда списывается с коэффициентом, и запас кончается
# примерно на седьмой минуте. Причина: бесплатная минута — реальные
# деньги за модель и голос, а те, кто доходит до стены, платят заметно
# чаще тех, кто до неё не дожил. Обещание при этом не нарушается: «десять
# минут» — это размер запаса, а не секундомер настенных часов, и
# показываемый остаток считается из того же запаса, так что цифра в бадже
# и реальность сходятся.
#
# 10 / 7 ≈ 1.43. Меняется здесь и нигде больше: клиент шлёт реально
# прошедшие секунды, коэффициент применяется на сервере, чтобы правка
# действовала сразу для всех и её нельзя было обойти из браузера.
USAGE_SPEED = float(os.environ.get("METER_USAGE_SPEED") or "1.43")


def billed_seconds(seconds: int) -> int:
    """Сколько секунд списать с запаса за `seconds` реального разговора."""
    return max(1, int(round(max(0, int(seconds)) * USAGE_SPEED)))
...
class SubscriptionMeter:
    ...
    async def record_usage(self, user_id: int, seconds: int) -> Dict[str, Any]:
        ...
        # Ускоренное списание: см. USAGE_SPEED.
        seconds = billed_seconds(seconds)
