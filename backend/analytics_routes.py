...
def register_analytics_routes(app, db):
    ...
    async def analytics_product(request: Request,
        ...
        _check_admin(x_admin_token)
        # Fail-soft: у каждой секции свои дефолты; если один (тяжёлый) запрос
        # падает или упирается в таймаут — вкладка всё равно рендерится с
        # остальными данными, а причина видна в out["_section_errors"].
        out: Dict[str, Any] = {
            "cohort": {
                "size": 0, "window": "первые визиты 8–38 дней назад",
                "d1_pct": None, "ret7_pct": None,
                "activation": {"sent_message_pct": None, "sent_3plus_pct": None,
                               "tried_game_pct": None},
            },
            "feature_lift": [],
            "depth": {
                "sessions_30d": 0, "zero_msg_pct": None, "median_msgs": None,
                "median_duration_sec": None,
                "sessions_per_user": {"one": 0, "two_four": 0, "five_plus": 0,
                                      "total_users": 0},
            },
            "games": [],
            "paywall": {"blocked_users": 0, "subscribe_clicked": 0, "checkout_opened": 0},
            "health_7d": [],
        }
        section_errors: Dict[str, str] = {}
        # Ограничение времени на запрос: тяжёлая когортная выборка не должна
        # вешать весь эндпоинт (и упираться в gateway-timeout).
        Q_TIMEOUT = 25.0
        try:
            async with db.get_connection() as conn:
                ...
                try:
                    ...
                    section_errors["cohort"] = f"{type(e).__name__}: {str(e)[:200]}"
                    logger.error(f"product cohort: {type(e).__name__}: {e}")
                ...
                try:
                    ...
                    section_errors["feature_lift"] = f"{type(e).__name__}: {str(e)[:200]}"
                    logger.error(f"product feature_lift: {type(e).__name__}: {e}")
                ...
                try:
                    ...
                    section_errors["depth"] = f"{type(e).__name__}: {str(e)[:200]}"
                    logger.error(f"product depth: {type(e).__name__}: {e}")
                ...
                try:
                    ...
                    section_errors["games"] = f"{type(e).__name__}: {str(e)[:200]}"
                    logger.error(f"product games: {type(e).__name__}: {e}")
                ...
                try:
                    ...
                    section_errors["paywall"] = f"{type(e).__name__}: {str(e)[:200]}"
                    logger.error(f"product paywall: {type(e).__name__}: {e}")
                ...
                try:
                    ...
                    section_errors["health_7d"] = f"{type(e).__name__}: {str(e)[:200]}"
                    logger.error(f"product health_7d: {type(e).__name__}: {e}")

                if section_errors:
                    out["_section_errors"] = section_errors
                return out
        except Exception as e:
