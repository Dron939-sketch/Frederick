...
const Test = {
    ...
    getStage2Interpretation() {
        ...
        const map = {
            'СОЦИАЛЬНО-ОРИЕНТИРОВАННЫЙ': {'1-3':'Ваше мышление конкретно и привязано к социальным ситуациям.','4-6':'Вы замечаете социальные закономерности и тренды.','7-9':'Вы видите глубинные социальные механизмы и законы.'},
            'СТАТУСНО-ОРИЕНТИРОВАННЫЙ':  {'1-3':'Ваше мышление направлено на достижение статуса.','4-6':'Вы стратегически мыслите в категориях статуса.','7-9':'Вы видите иерархические закономерности.'},
            'СМЫСЛО-ОРИЕНТИРОВАННЫЙ':    {'1-3':'Вы ищете смыслы в отдельных событиях.','4-6':'Вы находите закономерности в жизненных историях.','7-9':'Вы постигаете глубинные смыслы бытия.'},
            'ПРАКТИКО-ОРИЕНТИРОВАННЫЙ':  {'1-3':'Ваше мышление конкретно и практично.','4-6':'Вы видите практические закономерности.','7-9':'Вы создаёте эффективные практические модели.'}
        };
        return (map[this.perceptionType]||map['СОЦИАЛЬНО-ОРИЕНТИРОВАННЫЙ'])[g];
    },

    getStage3Interpretation() {
        const l = this.calculateFinalLevel();
        const fb = {1:'Ваше поведение реактивно.',2:'Вы начинаете осознавать свои автоматические реакции.',3:'Вы можете выбирать реакции, но не всегда.',4:'Вы управляете своим поведением в большинстве ситуаций.',5:'Поведение становится инструментом для достижения целей.',6:'Вы мастерски владеете своим поведением.'};
        const idx = l<=2?1:l<=4?2:l<=6?3:l<=8?4:l>=9?6:5;
        return fb[idx]||fb[3];
    },

    getStage5Interpretation() {
        const d = (this.deepPatterns||{attachment:'🤗 Надежный'}).attachment;
        const map = {'🤗 Надежный':'У тебя надёжный тип привязанности — ты уверен в отношениях и не боишься близости.','😥 Тревожный':'Тревожный тип привязанности: ты часто боишься, что тебя бросят.','🛡️ Избегающий':'Избегающий тип: ты держишь дистанцию, боишься близости.','🏔️ Отстраненный':'Отстранённый тип: ты обесцениваешь отношения.'};
    ...
    showStage2Result() {
        this.addMessageWithButtons('✨ РЕЗУЛЬТАТ ЭТАПА 2\n\nУровень мышления: '+this.thinkingLevel+'/9\n\n'+this.getStage2Interpretation()+'\n\n▶️ ЭТАП 3: КОНФИГУРАЦИЯ ПОВЕДЕНИЯ\n📊 Вопросов: 8\n⏱ Время: ~3 минуты', [
    ...
    showStage3Result() {
        ...
        this.addMessageWithButtons('✨ РЕЗУЛЬТАТ ЭТАПА 3\n\nВаши поведенческие уровни:\n• СБ: '+sb+'/6  • ТФ: '+tf+'/6  • УБ: '+ub+'/6  • ЧВ: '+cv+'/6\n\nФинальный уровень: '+this.calculateFinalLevel()+'/9\n\n'+this.getStage3Interpretation()+'\n\n▶️ ЭТАП 4: ТОЧКА РОСТА\n📊 Вопросов: 8', [
    ...
    showStage5Result() {
        this.addBotMessage('✨ РЕЗУЛЬТАТ ЭТАПА 5\n\n'+this.getStage5Interpretation()+'\n\n✅ ТЕСТ ЗАВЕРШЕН!\n\n🧠 Собираю воедино результаты 5 этапов...', true);
